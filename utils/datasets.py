...
def get_hash(files):
    # Returns a single hash value of a list of files # 返回文件列表的hash值
    return sum(os.path.getsize(f) for f in files if os.path.isfile(f))


def exif_size(img):
    # Returns exif-corrected PIL size 获取数码相机的图片宽高信息 并且判断是否需要旋转(数码相机可以多角度拍摄
...
class LoadImages:  # for inference
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            #img0 = letterbox(img0, self.img_size, stride=self.stride)[0]
...
#提早缓存图片到内存中，能够提高训练速度
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            if random.random() < 0.8:
                ...
                img, labels = load_mosaic9(self, index)#img调整为（640，640）
            shapes = None

            # MixUp https://arxiv.org/pdf/1710.09412.pdf
            if random.random() < hyp['mixup']:#Mixup: 将随机的两张样本按比例混合，分类的结果按比例分配；
                if random.random() < 0.8:
    ...
    def collate_fn(batch):
        ...
        for i, l in enumerate(label):
            l[:, 0] = i  # add target image index for build_targets()一个bach中的第几张图片
...
#对mosaic整合后的图片进行随机旋转、缩放、平移、裁剪，透视变换，并resize为输入大小 img_size。
