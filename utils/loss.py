...
#分配策略
def NWD_calculator(box1,box2, c=20):
    # centers of boxes
    ...
    centers_distance = (x_p - x_g) ** 2 + (y_p - y_g) ** 2
    ...
    w_gt = box2[:, 2] - box2[:, 0]
    h_gt = box2[:, 3] - box2[:, 1]
    wh_distance = ((w_pred - w_gt) ** 2 + (h_pred - h_gt) ** 2) / 4

    wassersteins = torch.sqrt(centers_distance + wh_distance)
...
class ComputeLossOTA:
    ...
    def __init__(self, model, autobalance=False):
        ...
        device = next(model.parameters()).device  # get model device
        h = model.hyp  # hyperparameters
#定义分类损失和置信度损失为带sigmoid的二值交叉熵损失，即会先将输入进行sigmoid再计算BinaryCrossEntropyLoss(BCELoss)。pos_weight参数是正样本损失的权重参数
        # Define criteria
        BCEcls = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([h['cls_pw']], device=device))
        BCEobj = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([h['obj_pw']], device=device))
#对标签做平滑,eps=0就代表不做标签平滑,那么默认cp=1,cn=0后续对正类别赋值cp，负类别赋值cn
        # Class label smoothing https://arxiv.org/pdf/1902.04103.pdf eqn 3
        self.cp, self.cn = smooth_BCE(eps=h.get('label_smoothing', 0.0))  # positive, negative BCE targets
        # self.C = h['c']#nwd的c参数3
        self.C = 3
        ...
        if g > 0:
            BCEcls, BCEobj = FocalLoss(BCEcls, g), FocalLoss(BCEobj, g)
#每一层预测值所占的权重比，分别代表浅层到深层，小特征到大特征，4.0对应着P3，1.0对应P4,0.4对应P5。如果是自己设置的输出不是3层，则返回[4.0, 1.0, 0.25, 0.06, .02]，可对应1-5个输出层P3-P7的情况。
        det = model.module.model[-1] if is_parallel(model) else model.model[-1]  # Detect() module
        self.balance = {3: [4.0, 1.0, 0.4]}.get(det.nl, [4.0, 1.0, 0.25, 0.06, .02])  # P3-P7
        self.ssi = list(det.stride).index(16) if autobalance else 0  # stride 16 index
        self.BCEcls, self.BCEobj, self.gr, self.hyp, self.autobalance = BCEcls, BCEobj, model.gr, h, autobalance#赋值各种参数,gr是用来设置IoU的值在objectness loss中做标签的系数,
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model   
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                # lbox += (1.0 - iou).mean()  # iou loss
                nwd = torch.exp(-torch.pow(Wasserstein(pbox.T, selected_tbox, x1y1x2y2=False), 1 / 2) / self.C)
                lbox += 1.0* (1.0 - iou).mean() + 0.0* (1.0 -nwd).mean()
        ...
        for batch_idx in range(p[0].shape[0]):
            ...
            for i, pi in enumerate(p):
               ...
               # from_which_layer.append(torch.ones(size=(len(b),)) * i)
                from_which_layer.append((torch.ones(size=(len(b),)) * i).to('cuda'))#修改
            ...
            #pair_wise_iou=NWD_calculator(txyxy, pxyxys c=20)
